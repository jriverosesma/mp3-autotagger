...
class MainWindow(qtw.QMainWindow, Ui_MainWindow):
    ...
    def manually_add_cover(self, event):
        selected_cover = qtw.QFileDialog.getOpenFileName(self, 'Select image', '', 'Image files (*.png; *.jpg; *.jpeg; *.bmp; *.tif; *.tiff; *.eps)')[0]
        if selected_cover:
    ...
    def find_all_mp3(self, find_path):
        ...
        for path, _, files in os.walk(find_path):
            filepaths.extend([os.path.join(path, file).replace('\\', '/') for file in files if str(file).endswith('.mp3')])
    ...
    def find_tags(self, thread_shazam=True, raise_exception=False):
        ...
        try:
            if thread_shazam:
                thread_update_tags_shazam = Thread(target=self.current_track.update_tags_shazam, kwargs={'replace_info': self.checkBox_replace_info.isChecked()})
    ...
    def download_yt_audio(self):
        ...
        error_text = None
        try:
            thread_download_youtube = Thread(target=download_youtube_audiostream, args=(selected_audiostream, self.download_folder), kwargs={'callback': self.callback_download_yt})
